export default async function testSpelling(pageSource) {
    // Simulate testing for spelling
    await new Promise(resolve => setTimeout(resolve, 600)); // Simulate async work
...
async function runTest(pageSource) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (!hasBasicWords) {
                console.warn('External dictionary missing basic words, using comprehensive fallback');
        ...
        console.warn('Could not load external dictionary, using fallback:', error.message);
        ...
        bodyWords.forEach(word => {
            if (!isSpelledCorrectly(word)) {
                bodyMisspellings.push({
                    word: word,
                    context: bodyText.substring(Math.max(0, bodyText.indexOf(word) - 20), bodyText.indexOf(word) + 30),
                    suggestion: commonMisspellings[word.toLowerCase()] || 'Check spelling'
                });
            }
        });
    ...
    const subTests = [
        {
            ...
            location: titleMisspellings.length === 0 ? 'Page title' : 
                titleMisspellings.map(m => `"${m.word}" in title`).join('\n• '),
        ...
        },
        {
            ...
            location: metadataMisspellings.length === 0 ? 'Metadata section' : 
                metadataMisspellings.map(m => `${m.key}: "${m.word}"`).join('\n• '),
        ...
        },
        {
            ...
            location: bodyMisspellings.length === 0 ? 'Main content area' : 
                bodyMisspellings.map(m => `"${m.word}" in content`).join('\n• '),
    ...
    const totalMisspellings = titleMisspellings.length + metadataMisspellings.length + bodyMisspellings.length;
